...
module tb_ahb_wrapper;
    ...
    // --- NEW: Wires for the unidirectional DUT ports ---
    ...
    // --- NEW: Testbench Physical Pad Simulation ---
    // This acts exactly like the physical tri-state pad on a chip's I/O ring.
    // When the DUT wants to output (oen=1), it drives the bidirectional qspi_io wire.
    // Otherwise, it releases the wire to High-Z so the flash model can drive it.
    assign qspi_io = dut_qspi_data_oen ? dut_qspi_data_out : 4'bz;
    
    // The DUT always listens to whatever is currently on the qspi_io wire.
    assign dut_qspi_data_in = qspi_io;


    // --- FLASH MODEL (Unchanged) ---
    flash_model u_flash (
    ...
    initial begin
        $dumpfile("ahb_sys.vcd");
        $dumpvars(0, tb_ahb_wrapper);
    end
