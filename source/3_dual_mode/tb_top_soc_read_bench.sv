...
module tb_soc_top();
    ...
    // ==========================================
    initial begin
        $fsdbDumpfile("soc_dual_read_bench.fsdb");
        $fsdbDumpvars(0, tb_soc_top);
        $fsdbDumpMDA();
        #100000;
        $error("[DUAL READ BENCH] TIMEOUT before common read workload completed.");
        $finish;
    end

    // ==========================================
    // 8. Common Dataset Read Timing Monitor
    // ==========================================
    ...
    time         first_read_done_time;
    time         all_reads_done_time;
    time         done_marker_time;
    ...
    initial begin
        ...
        first_read_done_time = 0;
        all_reads_done_time = 0;
        done_marker_time = 0;
        wait (HRESETn === 1'b1);
        ...
        if (HRESETn && SYS_HREADY) begin
            if (flash_read_active) begin
                if (SYS_HRDATA !== expected_flash_data(flash_read_addr)) begin
                    $error("[DUAL READ BENCH] Read mismatch at %08h: expected %08h, got %08h",
                           flash_read_addr, expected_flash_data(flash_read_addr), SYS_HRDATA);
                    $finish;
                end

                read_count = read_count + 1;
                $display("[%0t] [DUAL READ BENCH] Read %0d complete: %08h -> %08h",
                         $time, read_count, flash_read_addr, SYS_HRDATA);

                if (read_count == 1) first_read_done_time = $time;
                if (read_count == 4) all_reads_done_time = $time;
            end

            if (done_write_active) begin
                done_marker_time = $time;
                if (SYS_HWDATA !== 32'haabb_ccdd) begin
                    $error("[DUAL READ BENCH] Completion marker mismatch: expected aabbccdd, got %08h", SYS_HWDATA);
                    $finish;
                end
                if (read_count != 4) begin
                    $error("[DUAL READ BENCH] Done marker observed after %0d reads, expected 4.", read_count);
                    $finish;
                end

                $display("\n*******************************************************");
                $display("[%0t] DUAL MODE READ BENCH SUCCESS: COMMON 16-BYTE DATASET PASSED", $time);
                $display("First read latency : %0t ps", first_read_done_time - reset_release_time);
                $display("4-word read time   : %0t ps", all_reads_done_time - reset_release_time);
                $display("Post-read latency  : %0t ps", done_marker_time - all_reads_done_time);
                $display("Total boot-to-done : %0t ps", done_marker_time - reset_release_time);
                $display("*******************************************************\n");
                #100; $finish;
            end
